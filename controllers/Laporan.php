...
class Laporan extends CI_Controller {
	...
	if($this->session->userdata('level')=='admin') {
		...
		$where = array('nonota' => $id );
		$data['status'] = $this->M_laporan->edit($where,'tbl_penjualan')->result();
		$this->load->view('modal/edit',$data);
	}

	public function update() {
		$id 		= $this->input->post('nonota',TRUE);
		...
		$where = array('nonota' => $id );
		$this->M_laporan->update($where,$data,'tbl_penjualan');
